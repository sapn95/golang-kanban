...
ALTER TABLE boards
	ADD COLUMN sla_response_hours INTEGER NOT NULL DEFAULT 0 CHECK (sla_response_hours >= 0),
	ADD COLUMN sla_days INTEGER NOT NULL DEFAULT 62 CHECK (sla_days BETWEEN 0 AND 127),
	ADD COLUMN sla_start INTEGER NOT NULL DEFAULT 480 CHECK (sla_start BETWEEN 0 AND 1440),
	ADD COLUMN sla_end INTEGER NOT NULL DEFAULT 1020 CHECK (sla_end BETWEEN 0 AND 1440),
	-- An IANA zone name, empty for UTC. The office hours of a desk are local
	-- hours, so a clock that ran in UTC would open an hour late for half the
	-- year.
	ADD COLUMN sla_zone TEXT NOT NULL DEFAULT '';
...
ALTER TABLE columns ADD COLUMN stops_clock BOOLEAN NOT NULL DEFAULT FALSE;
